//! OMT Studio Monitor (GPUI) — temporary alternate UI for A/B comparison.
//!
//! Headless performance runs:
//! ```text
//! cargo run --release -p omt-studio-monitor-gpui -- --headless --url omt://... --seconds 10
//! ```
//!
//! Compare against the egui binary:
//! ```text
...
#[derive(Debug, Parser)]
#[command(
    name = "omt-studio-monitor-gpui",
    about = "OMT Studio Monitor (GPUI prototype)"
)]
struct Args {
    /// UI language (`en` / `ja`).
    #[arg(long)]
...
fn main() -> anyhow::Result<()> {
    ...
    let title = format!(
        "{} (GPUI)",
        t(overrides.language, "tool.studio_monitor")
    );
